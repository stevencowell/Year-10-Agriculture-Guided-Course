(function () {
  ...
  function referencePhotosHtml(section) {
    if (!section.photos?.length) return "";
    return `<div class="tool-photo-gallery" aria-label="Supporting source visuals">${section.photos.map((photo) => `<figure class="tool-photo-card"><a class="tool-photo-link zoomable-infographic" href="${esc(photo.image)}" target="_blank" rel="noopener" aria-label="Open supporting visual in a new tab: ${esc(photo.alt)}"><img src="${esc(photo.image)}" alt="${esc(photo.alt)}"><span class="infographic-open-label">Open larger <span aria-hidden="true">↗</span></span></a><figcaption>${esc(photo.caption)} <span class="tool-photo-credit">${photo.source ? `<a href="${esc(photo.source)}" target="_blank" rel="noopener">${esc(photo.credit)}</a>` : esc(photo.credit)}</span></figcaption></figure>`).join("")}</div>`;
  }

  function drawingGuidanceHtml(section) {
    const guidance = section.drawingGuidance;
    if (!guidance) return "";
    const id = `drawing-guidance-${section.id.replace(".", "-")}`;
    const sheets = guidance.sheets.map((sheet) => {
      const alreadyShown = section.visual?.image === sheet.preview;
      const sourceLink = sheet.sourceUrl ? `<span><a href="${esc(sheet.sourceUrl)}" target="_blank" rel="noopener">Authorised Drive source ↗</a></span>` : "";
      if (alreadyShown) {
        return `<article class="plan-sheet-card"><p><strong>${esc(sheet.title)}</strong></p><p>${esc(sheet.caption)}</p><p>The purposeful full-resolution guide is shown once above; this drawing-reading block adds interpretation rather than repeating the same image for decoration.</p>${sourceLink}<p class="plan-actions"><a class="plan-open-link" href="${esc(sheet.open)}" target="_blank" rel="noopener">Open larger diagram <span aria-hidden="true">↗</span></a></p></article>`;
      }
      return `<figure class="plan-sheet-card"><a class="plan-preview-link zoomable-infographic" href="${esc(sheet.open)}" target="_blank" rel="noopener" aria-label="Open larger diagram in a new tab: ${esc(sheet.title)}"><img src="${esc(sheet.preview)}" alt="${esc(sheet.alt)}"><span class="infographic-open-label">Open larger <span aria-hidden="true">↗</span></span></a><figcaption><strong>${esc(sheet.title)}</strong><span>${esc(sheet.caption)}</span>${sourceLink}<span class="plan-actions"><a class="plan-open-link" href="${esc(sheet.open)}" target="_blank" rel="noopener">Open larger diagram <span aria-hidden="true">↗</span></a></span></figcaption></figure>`;
    }).join("");
    return `<section class="plan-guidance" aria-labelledby="${id}"><p class="eyebrow">Diagram reading</p><h3 id="${id}">${esc(guidance.heading)}</h3>${guidance.paragraphs.map((paragraph) => `<p>${esc(paragraph)}</p>`).join("")}<h4>Drawing-reading takeaways</h4><ul>${guidance.takeaways.map((item) => `<li>${esc(item)}</li>`).join("")}</ul><div class="plan-sheet-gallery">${sheets}</div></section>`;
  }
  ...
  function theoryHtml(section, index, moduleNumber) {
    ...
    return `<section class="card theory-section" id="theory-${moduleNumber}-${index + 1}" tabindex="-1">
      ...
      ${drawingGuidanceHtml(section)}
      ${referencePhotosHtml(section)}
  ...
  function checksHtml(module, moduleNumber, theoryIndex) {
    ...
    const groups = [module.sections[theoryIndex]].map((section) => {
    ...
    return `<section class="card theory-section" id="${id}"><p class="eyebrow">Knowledge checks</p><h2>Ten questions for every theory section</h2><p>Complete each source-grounded set, use the hints when needed, and check your feedback before moving to the written evidence.</p>${groups}</section>`;
  }
  ...
  function bindFolio() {
    ...
    cards.forEach((card, index) => {
      ...
      visualLink.setAttribute("aria-label", `Open larger: ${visual.getAttribute("aria-label")}`);
